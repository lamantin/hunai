...
from client import hunaipath
from client import diagnose
from client.conversation import Conversation

# Add jasperpath.LIB_PATH to sys.path
sys.path.append(hunaipath.LIB_PATH)

parser = argparse.ArgumentParser(description='Hunai Voice Control Center')
...
class Hunai(object):
    def __init__(self):
        ...
        if not os.path.exists(hunaipath.CONFIG_PATH):
            try:
                os.makedirs(hunaipath.CONFIG_PATH)
            except OSError:
                self._logger.error("Could not create config dir: '%s'",
                                   hunaipath.CONFIG_PATH, exc_info=True)
                raise

        # Check if config dir is writable
        if not os.access(hunaipath.CONFIG_PATH, os.W_OK):
            self._logger.critical("Config dir %s is not writable. Jasper " +
                                  "won't work correctly.",
                                  hunaipath.CONFIG_PATH)

        # FIXME: For backwards compatibility, move old config file to newly
        #        created config dir
        old_configfile = os.path.join(hunaipath.LIB_PATH, 'profile.yml')
        new_configfile = hunaipath.config('profile.yml')
    ...
    def run(self):
        ...
        conversation = Conversation("HUNAI", self.mic, self.config)
        conversation.handleForever()

if __name__ == "__main__":

    print("*******************************************************")
    print("*HUNAI - THE TALKING COMPUTER IN HUNGARIAN SPEAKING BASED ON JASPER *")
    print("* (c) 2018 Makai István *")
    print("*******************************************************")
    ...
    if not args.no_network_check and not diagnose.check_network_connection():
        logger.warning("Network not connected. This may prevent Hunai from " +
                       "running properly.")
